...
* Built on **open-source Kubernetes**(2014)
...
### 4. Cloud Functions
* Falls under FaaS - Function as a Service
...
### 1. Cloud Storage
* Consistent, scalable, large-capacity, highly durable **object storage**(Managing data as object such as documents or pictures)
* **11 9’s Durability** (99.999999999%)(Basically cloud storage is a very good place to store the file)
...
* Block storage is different from object storage
* Standard – Regular standard storage at a reasonable price
* Solid State (SSD) - Lower latency/higher IOPS
* Both Standard and SSD are available in zonal and regional options.

| **Feature**          | **Block Storage**                                       | **File Storage**                      | **Object Storage**                               |
| -------------------- | ------------------------------------------------------- | ------------------------------------- | ------------------------------------------------ |
| **Data Structure**   | Fixed-size blocks with addresses                        | Hierarchical folders and files        | Whole objects with metadata and unique IDs       |
| **Access Method**    | Through OS as raw disk (e.g., via iSCSI, Fibre Channel) | Through file paths (e.g., NFS, SMB)   | Through APIs or HTTP using object keys           |
| **Performance**      | Low-latency, high performance                           | Moderate, depends on file system      | Optimized for scalability, not latency           |
| **Best For**         | Databases, VMs, transactional systems                   | Shared file systems, home directories | Backups, media archives, big data                |
| **Scalability**      | Limited by attached volume size                         | Limited by file system size           | Virtually unlimited                              |
| **Metadata Support** | Minimal (block ID only)                                 | Basic (file name, size, permissions)  | Rich metadata per object                         |
| **Examples**         | AWS EBS, local SSD/HDD, SAN                             | Windows File Server, NFS shares       | AWS S3, Azure Blob Storage, Google Cloud Storage |

Quick Analogy -  
* **Block storage** = Like a blank notebook — you can write anything in any order, but you have to keep track of the pages yourself.

* **Object storage** = Like a library — each “book” (object) has a title, description, and is stored in a catalog system for retrieval.
